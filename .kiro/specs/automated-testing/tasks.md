...
- **Follow-up 実機検証で発覚した upstream issues**:
  - `drawio-file-io`: `readDrawioXml` の `<mxfile>` 早期 return を撤去し `<diagram>` 子要素 base64 を inflate するよう修正済 (本 spec 内で fix)。
  - `drawio-embed-bridge`: iframe に `data-drawio` 属性追加、`createDrawioBridge(app, pluginDir)` で plugin-relative path に変更 (本 spec 内で fix)。
  - **drawio-embed-bridge**: Obsidian の `app://` protocol handler が drawio webapp の sub-resource (js/main.js / styles/grapheditor.css 等) を `ERR_BLOCKED_BY_CLIENT` で拒否するため、iframe が bootstrap せず init message が観測できない。`tests/e2e/drawio-iframe-init.spec.ts` / `three-formats-roundtrip.spec.ts` / `external-sync-reload.spec.ts` を `test.fixme` 化。upstream で resource serving 戦略 (file:// or 専用 protocol or asset 同梱) を見直す別 spec を要立ち上げ。
- **E2E 起動戦略**: `--user-data-dir` をテストごとに ` mkdtempSync` で個別 provision し、setup の canonical state を copy してから起動することで Obsidian の SingletonLock 競合を回避。`--lang=en-US` で UI ロケールを EN に固定。Obsidian 自体は `OBSIDIAN_APP_PATH` (default `/Applications/Obsidian.app`) または `--ci` で `gh release download` した `.obsidian-app/Obsidian.app` から `app.asar` を抽出。Playwright `_electron.launch` は CDP 対応の npm electron で起動し、抽出済 `main.js` を args[0] に渡す。
- **drawio dir は symlink 不可**: Obsidian の `app://` protocol が symlink を辿らないため、`plugin-install` helper では drawio dir を `cpSync` で copy する。main.js / manifest.json / styles.css は symlink で OK。
