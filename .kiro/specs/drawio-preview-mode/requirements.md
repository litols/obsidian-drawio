...
6. The Drawio View shall プレビュー領域をビューのコンテンツ領域全体に広げて表示する (図はその中でフィット表示する)
7. When ユーザーがトラックパッドのピンチ操作または修飾キー+ホイール操作を行ったとき, the Drawio View shall プレビューの表示形式 (内包画像 / 図データ描画) に依らずカーソル位置を基準に図を拡大縮小する
8. When ユーザーがドラッグ操作または 2 本指スクロール操作を行ったとき, the Drawio View shall プレビューの表示形式に依らず図の表示位置を移動する
...
5. When エディタを起動したとき, the Drawio Plugin shall アセット転送に伴う一時メモリ使用量を分割配信によって抑制し、メモリ逼迫によるアプリの異常終了を生じさせない
6. While 重量アセット群 (図形ライブラリ等) の配信が完了していない間, the Drawio Editor shall 基本的な編集機能を利用可能な状態で表示する
...
4. The Drawio Plugin shall 設定画面でプレビューの背景色を変更する手段を提供する
5. The Drawio Plugin shall プレビュー背景色の初期値を白とする
6. When プレビュー背景色の設定を変更した後にプレビューを表示または再描画したとき, the Drawio View shall 変更後の背景色でプレビューを表示する
